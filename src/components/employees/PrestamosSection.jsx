...
const EMPTY = { date: today(), amount: '', notes: '' };

const PrestamosSection = ({ isAdmin, selectedEmployeeId }) => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await getLoans(isAdmin ? selectedEmployeeId : null);
      setItems(data.items || []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        setSuccess('Préstamo registrado');
      }
      setShowForm(false);
      setEditingId(null);
      setForm(EMPTY);
  ...
  const handleEdit = (item) => {
    setForm({ date: item.date, amount: String(item.amount), notes: item.notes || '' });
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-semibold text-gray-900">Préstamos</h2>
            <p className="text-xs text-gray-500">Dinero prestado con cargo a quincena</p>
          </div>
        </div>
        <button
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center gap-4">
        <div>
          <p className="text-xs text-amber-700 font-medium uppercase tracking-wide">Total prestado</p>
          <p className="text-2xl font-bold text-amber-800">{fmt(total)}</p>
        </div>
        <div className="ml-auto text-right">
          <p className="text-xs text-amber-600">{items.length} registro{items.length !== 1 ? 's' : ''}</p>
        </div>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-gray-50 border border-gray-200 rounded-xl p-5 space-y-4">
          <h3 className="font-semibold text-gray-800 text-sm">{editingId ? 'Editar préstamo' : 'Nuevo préstamo'}</h3>
