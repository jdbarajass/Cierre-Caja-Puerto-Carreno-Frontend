...
const calcDays = (s, e) => { try { const d = (new Date(e) - new Date(s)) / 86400000 + 1; return d > 0 ? d : 0; } catch { return 0; } };

const VacacionesSection = ({ isAdmin, filterNombre }) => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const data = await getVacations(filterNombre || null); setItems(data.items || []); setTotalDays(data.total_dias || 0); }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = { nombre_empleada: form.nombre_empleada.trim(), start_date: form.start_date, end_date: form.end_date, notes: form.notes };
      if (editingId) { await updateVacation(editingId, payload); setSuccess('Actualizado'); }
      else { await createVacation(payload); setSuccess('Registrado'); }
  ...
  const handleEdit = (item) => {
    setForm({ nombre_empleada: item.nombre_empleada, start_date: item.start_date, end_date: item.end_date, notes: item.notes || '' });
    setEditingId(item.id); setShowForm(true);
  };

  const handleDelete = async (id) => {
    if (!window.confirm('¿Eliminar?')) return;
    try { await deleteVacation(id); await load(); } catch (e) { setError(e.message); }
  };
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-teal-100 rounded-xl flex items-center justify-center"><Sun className="w-5 h-5 text-teal-600" /></div>
          <div><h2 className="text-base font-semibold text-gray-900">Vacaciones</h2><p className="text-xs text-gray-500">Períodos de vacaciones tomadas</p></div>
        </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-gray-50 border border-gray-200 rounded-xl p-5 space-y-4">
          <h3 className="font-semibold text-gray-800 text-sm">{editingId ? 'Editar' : 'Nuevas vacaciones'}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">Empleada *</label>
              <EmployeeSelect value={form.nombre_empleada} focusRing="focus:ring-teal-300"
          ...
          <div className="flex items-center gap-2 px-4 py-3 bg-teal-100 rounded-lg">
            <span className="text-sm text-teal-700 font-medium">Días:</span>
